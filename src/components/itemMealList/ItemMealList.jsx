...
export const ItemMealList = ({item, mealType}) => {
    ...
    const remove = () => {
        if (mealType === dietCtx.breakfast) {
            dietCtx.removeItemBreakfast(item.id)
        } else if (mealType === dietCtx.launch) {
            dietCtx.removeItemLaunch(item.id)
        } else if (mealType === dietCtx.dinner) {
            dietCtx.removeItemDinner(item.id)
        } else if (mealType === dietCtx.snacks) {
            dietCtx.removeItemSnacks(item.id)
        }
    }
    ...
    return (
        <div className="itemDiet" key={item.id}>
            <div className="titleContainerItemDiet">
                <div className="nutrientsDiet itemName">
                    <span>{item.name}</span>
                    <span className="grams">{item.grams.toFixed(1)} g</span>
                </div>
                <div className="nutrientsDiet">
                    <span className="number">{item.kcal.toFixed(1)}</span>
                </div>
            </div>
            <div className="nutrientsDietContainer">
                <div className="nutrientsDiet">
                    <span className="number">{item.protein.toFixed(1)} g</span>
                </div>
                <div className="nutrientsDiet">
                    <span className="number">{item.carbs.toFixed(1)} g</span>
                </div>
                <div className="nutrientsDiet">
                    <span className="number">{item.fat.toFixed(1)} g</span>
                </div>
                <div onClick={remove} style={{height:'30px', width:'80px', textAlign: 'center'}}>
                    {deleteIcon}
                </div>
            </div>
        </div>
